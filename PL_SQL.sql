...
--CRETE A TRIGGER THAT UPDATES INV_QOH AFTER A ROW IS INSERTED
--INTO THE ORDER_LINE TABLE
--Trigger Header
CREATE OR REPLACE TRIGGER CLEARWATER.UP_INV_AFTER_ORDER 
AFTER INSERT ON CLEARWATER.ORDER_LINE 
FOR EACH ROW
--Trigger Body
DECLARE
  L_QTY CLEARWATER.ORDER_LINE.OL_QUANTITY%TYPE := :NEW.OL_QUANTITY;
  L_INV_ID CLEARWATER.ORDER_LINE.INV_ID%TYPE :=  :NEW.INV_ID;

BEGIN
--determine the transaction type
  UPDATE CLEARWATER.INVENTORY
  SET INV_QOH = INV_QOH - :NEW.OL_QUANTITY
  WHERE INV_ID = L_INV_ID;  --NEW.INV_ID IS FROM TRIGGERING ORDER_LINE

END;

SELECT * FROM DBA_TRIGGERS
WHERE OWNER = 'CLEARWATER'

--CHECK OUT THE DATA
SELECT * FROM CLEARWATER.INVENTORY
WHERE INV_QOH IS NOT NULL

--BUILD TEST DATA
--AFTER THIS INSERT, INVENTORY.INV.QOH SHOULD SHOW 6 ITEMS LEFT IN INV_QOH
INSERT INTO CLEARWATER.ORDER_LINE VALUES (2, 1, 10)
SELECT INV_QOH FROM CLEARWATER.INVENTORY WHERE INV_ID = 1


--CREATE A TRIGGER THAT UPDATES INV_QPH AFTER A ROW(SL_QUANTITY) IS INSERTED
--INTO THE SHIPMENT_LINE TABLE

CREATE OR REPLACE TRIGGER CLEARWATER.UP_INV_AFTER_SHIPMENT
AFTER INSERT ON CLEARWATER.SHIPMENT_LINE
FOR EACH ROW
--Trigger Body
DECLARE
  L_QTY CLEARWATER.SHIPMENT_LINE.SL_QUANTITY%TYPE := :NEW.SL_QUANTITY;
  L_INV_ID CLEARWATER.ORDER_LINE.INV_ID%TYPE :=  :NEW.INV_ID;
...
--determine the transaction type
  ...
  WHERE INV_ID = L_INV_ID;  
  DBMS_OUTPUT.PUT_LINE('THE INV_QOH HAS BEEN UPDATED.');
...
--BUILD TEST DATA
--AFTER THIS INSERT, INVENTORY.INV.QOH SHOULD SHOW 100 ITEMS LEFT IN INV_QOH
INSERT INTO CLEARWATER.SHIPMENT_LINE VALUES
(6, 12, 100, TO_DATE('02/17/2022', 'MM/DD/YYYY'));
SELECT INV_QOH FROM CLEARWATER.INVENTORY WHERE INV_ID = 12

SELECT * FROM CLEARWATER.SHIPMENT_LINE
SELECT * FROM CLEARWATER.INVENTORY
